...
tfile=$(realpath chr_assemblies/${target}.pri.chr.fa)
qfile=$(realpath chr_assemblies/${query}.pri.chr.fa)
...
if [ ! -s ${target}_${query}.bam ]; then
    ...
    minimap2 -t 10 -ax asm5 -H -f 100 --rmq=no --secondary=no --eqx ${tfile} ${qfile} | samtools view -@ 10 -bS - | samtools sort -@ 10 -o ${target}_${query}.bam 2> ${target}_${query}.initialmap.log
    conda deactivate
fi 
...
if [ ! -s ${target}_${query}.reorient.bam ]; then
    ...
    source activate syri
    # Re-run minimap2
    minimap2 -t 10 -ax asm5 -H -f 100 --rmq=no --secondary=no --eqx ${target}_${query}fixchr.ref.filtered.fa ${target}_${query}fixchr.qry.filtered.fa | samtools view -@ 10 -bS - | samtools sort -@ 10 -o ${target}_${query}.reorient.bam 2> ${target}_${query}.reorientmap.log
fi 
